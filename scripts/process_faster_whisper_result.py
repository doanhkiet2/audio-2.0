...
# Cho phép bỏ qua tối đa N từ đầu expected nếu từ đầu bị faster-whisper nghe sai
ENABLE_PREFIX_RECOVERY = True
MAX_PREFIX_SKIP = 2

# Bật nếu muốn test strict số lượng từ
ENABLE_WORD_COUNT_REJECT = True
MAX_WORD_COUNT_DIFF = 0

# Vì đã có prefix recovery, first word có thể đặt cao
MIN_FIRST_WORD_SCORE = 85
MIN_LAST_WORD_SCORE = 65
...
def first_last_word_score(expected_words, span_words):
...
def find_best_word_span(
    ...
    expected_words = normalize_word_list(expected_text)
    expected_norm = join_words(expected_words)
    ...
    prefix_skip_values = [0]

    if ENABLE_PREFIX_RECOVERY:
        for n in range(1, MAX_PREFIX_SKIP + 1):
            if n < expected_word_count:
                prefix_skip_values.append(n)

    word_count_reject_count = 0
    first_word_reject_count = 0
    last_word_reject_count = 0
    time_reject_count = 0
    duration_reject_count = 0

    for prefix_skip in prefix_skip_values:
        core_expected_words = expected_words[prefix_skip:]
        core_expected_norm = join_words(core_expected_words)
        core_expected_word_count = len(core_expected_words)

        min_len = max(1, int(core_expected_word_count * 0.75))
        max_len = max(1, int(core_expected_word_count * 1.25))

        for i in range(len(words)):
            for j in range(i + min_len, min(len(words), i + max_len) + 1):
                core_span_words = words[i:j]
                core_span_word_count = len(core_span_words)

                # if ENABLE_WORD_COUNT_REJECT:
                #     if (
                #         abs(core_span_word_count - core_expected_word_count)
                #         > MAX_WORD_COUNT_DIFF
                #     ):
                #         word_count_reject_count += 1
                #         continue

                # Nếu bỏ qua 1 từ đầu expected thì cần recover lùi lại 1 từ faster
                recover_i = i - prefix_skip

                if recover_i < 0:
                    continue

                recovered_span_words = words[recover_i:j]

                span_start = recovered_span_words[0]["start"]
                span_end = recovered_span_words[-1]["end"]
                ...
                if span_duration < min_duration or span_duration > max_duration:
                    duration_reject_count += 1
                    continue

                first_score, last_score = first_last_word_score(
                    core_expected_words,
                    core_span_words,
                )

                if first_score < MIN_FIRST_WORD_SCORE:
                    first_word_reject_count += 1
                    continue

                if last_score < MIN_LAST_WORD_SCORE:
                    last_word_reject_count += 1
                    continue

                core_span_text = " ".join(w["word"] for w in core_span_words)
                core_span_norm = normalize_text(core_span_text)

                recovered_span_text = " ".join(w["word"] for w in recovered_span_words)
                recovered_span_norm = normalize_text(recovered_span_text)

                # Score chính vẫn so full expected với full recovered span
                text_score = fuzz.ratio(expected_norm, recovered_span_norm)
                core_text_score = fuzz.ratio(core_expected_norm, core_span_norm)
                ...
                avg_prob = get_avg_prob(recovered_span_words)
                max_word_gap = get_max_word_gap(recovered_span_words)

                if best is None or score > best["score"]:
                    best = {
                        "score": round(score, 2),
                        "text_score": round(text_score, 2),
                        "core_text_score": round(core_text_score, 2),
                        "first_word_score": round(first_score, 2),
                        "last_word_score": round(last_score, 2),
                        "prefix_skip": prefix_skip,
                        "prefix_expected_skipped": join_words(
                            expected_words[:prefix_skip]
                        ),
                        "expected_core_text": join_words(core_expected_words),
                        "matched_core_text": core_span_text,
                        "matched_text": recovered_span_text,
                        "avg_word_probability": (
                            None if avg_prob is None else round(avg_prob, 4)
                        ),
                        "max_word_gap": round(max_word_gap, 3),
                        "word_start_index": recover_i,
                        "word_core_start_index": i,
                        "word_end_index": j - 1,
                        ...
                        "core_word_count": len(normalize_text(core_span_text).split()),
                        "expected_core_word_count": len(core_expected_norm.split()),
                        "word_count": len(normalize_text(recovered_span_text).split()),
    ...
    if best is None:
        reason = (
            "NO_GOOD_SPAN"
            f"|FIRST_WORD_REJECT={first_word_reject_count}"
            f"|LAST_WORD_REJECT={last_word_reject_count}"
            f"|TIME_REJECT={time_reject_count}"
            f"|DURATION_REJECT={duration_reject_count}"
        )
        return None, reason
...
def judge_result(best, new_start, new_end, original_start, original_end):
    ...
    if ENABLE_WORD_COUNT_REJECT:
        faster_count = best.get("core_word_count", 0)
        expected_count = best.get("expected_core_word_count", 0)
        word_count_diff = abs(
            best.get("core_word_count", 0) - best.get("expected_core_word_count", 0)
        )

        if word_count_diff > MAX_WORD_COUNT_DIFF:
            reasons.append(f"WORD_COUNT_DIFF={word_count_diff}")
            print("\n[WORD COUNT REJECT]")
            print(f"index              : {best.get('index')}")
            print(f"faster_count       : {faster_count}")
            print(f"expected_count     : {expected_count}")
            print(f"word_count_diff    : {word_count_diff}")
            print(f"prefix_skip        : {best.get('prefix_skip')}")
            print(f"faster_matched_text: {best.get('matched_text')}")
            print(f"text_expected      : {best.get('expected_full_text')}")
            print("-" * 80)
    ...
    with open(FASTER_WHISPER_RESULT, "r", encoding="utf-8") as fin, open(
        ...
        for line in tqdm(fin, desc="Processing faster-whisper result"):
            ...
            out_row = {
                ...
                "core_text_score": best.get("core_text_score"),
                "match_score": best["score"],
                "first_word_score": best.get("first_word_score"),
                "last_word_score": best.get("last_word_score"),
                "prefix_skip": best.get("prefix_skip"),
                "prefix_expected_skipped": best.get("prefix_expected_skipped"),
                "expected_core_text": best.get("expected_core_text"),
                "matched_core_text": best.get("matched_core_text"),
                ...
                "core_word_count": best.get("core_word_count"),
                "expected_word_count": best.get("expected_word_count"),
                "expected_core_word_count": best.get("expected_core_word_count"),
                "text_expected": expected_text,
                "faster_text_pred": row.get("text_pred"),
                "faster_matched_text": best["matched_text"],
                "word_start_index": best["word_start_index"],
                "word_core_start_index": best["word_core_start_index"],
